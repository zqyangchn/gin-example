...
func (s *Registry) Get(store Store, name string) (*Session, error) {
	if !isCookieNameValid(name) {
		return nil, errors.Errorf("sessions: invalid character in cookie name: %s", name)
	}
...
func (s *Registry) Save(w http.ResponseWriter) error {
	...
	for name, info := range s.sessions {
		...
		if session.store == nil {
			errMulti = append(errMulti, errors.Errorf("sessions: missing store for session %q", name))
			continue
		}
		if err := session.store.Save(s.request, w, session); err != nil {
			errMulti = append(errMulti, errors.Errorf("sessions: error saving session %q -- %v", name, err))
		}
	}
	if errMulti != nil {
		return errMulti
...
func (m MultiError) Error() string {
	...
	return errors.Errorf("%s (and %d other errors)", s, n-1).Error()
}
